    ...
    <div className="mx-auto w-full max-w-3xl space-y-8 pb-20">
      ...
      {displayPosts.length > 0 ? (
        displayPosts.map((post, index) => {
          ...
          const isTopTrending = !targetPostId && index < 3;
          ...
          return (
            ...
            >
              {isTopTrending && (
                <div className="absolute right-3 top-2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-orange-500 text-xs font-bold text-white">
                  #{index + 1}
                </div>
              )}
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="flex h-15 w-15 items-center justify-center rounded-full bg-emerald-500/10 ring-1 ring-emerald-500/15 backdrop-blur-sm dark:bg-emerald-500/10">
            <Flame className="h-7 w-7 text-emerald-600 dark:text-emerald-400" />
          </div>
          ...
          <p className="mt-1 max-w-xs text-sm text-muted-foreground">
            Posts need a few likes within the last 30 days to show up here. Be
            the first to like something today!
          </p>
        </div>
      )}
